...
mongoose.connect("mongodb://localhost:27017/feed",{userNewUrlParser:true});
...
for(var j = 0,i=0 ;i<1000;i++){
  j = j%100;
  j++;
  var myDate = candlestickData[j].time;
  myDate = myDate.split("-");
  var newDate = new Date( myDate[2], myDate[1] - 1, myDate[0]);
  y.t.push(new Date(newDate).getTime()/1000);
  // y.t.push(time);
  // console.log(newDate.getTime()/1000);
  y.c.push(candlestickData[j].close);
  y.o.push(candlestickData[j].open);
  y.h.push(candlestickData[j].high);
  y.l.push(candlestickData[j].low);

  time += 1000;
}
const Feed = mongoose.model("Feed",feedSchema);
// const _data = new Feed(y);
// console.log(_data);
